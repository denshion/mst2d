function [Cext] = CextIntegrateFields(ki, r_sca, a, l, Rint, polarization, gg_)
% CextIntegrateFields(ki, r_sca, a, Rint, polarization, [gg = 100])
%   Compute the extinction cross-section by integrating the scattered fields
...
%   should be large. Default value: s = 100.
  ...
  for iter = 1:numel(angles)
    ...
    Hi = exp(1i .* dot(ki, r)) .* Hi_direction;
    Es = ScatteredField .* polarization;
    Hs = ScatteredField .* Hs_direction;
    
    integrand(iter) = -dot(r_3d, real(cross(Es, conj(Hi)) + cross(conj(Ei), Hs)));
  end
